...
namespace AslLabApi.Controllers
{
    public class ClosingBalanceController : AppController
    {
        ...
        public ActionResult IndexPost(PageModel model, string command)
        {
            ...
            if (command == "Submit")
            {
                if (model.AGlMaster.TRANSDT != null)
                {
                   ...
                   // string converttoString1 = Convert.ToString(changedtxt.ToString("dd-MM-yyyy"));
                    ...
                    var result = db.GlMasterDbSet.Count(d => d.TRANSTP == "OPEN" && d.COMPID == companyID && d.TRANSDT==model.AGlMaster.TRANSDT);
                    ...
                    {
                        var check_date = db.GlMasterDbSet.Count(d => model.AGlMaster.TRANSDT == d.TRANSDT && d.COMPID == companyID && d.TRANSTP=="OPEN");
                        //var x =
            ...
            if (command == "Add")
            {
                if (model.AGlMaster.TRANSDT != null && (model.AGlMaster.DEBITCD!=0 && model.AGlMaster.DEBITCD!=null))
                {
                    //Get Ip ADDRESS,Time & user PC Name
                    ...
                    System.Data.SqlClient.SqlConnection conn =
                        new System.Data.SqlClient.SqlConnection(
                            System.Configuration.ConfigurationManager.ConnectionStrings["AslLabApiContext"].ToString());
                    string query =
                        string.Format(
                            "SELECT STATUS, INSERTR, UPDATER, DELETER FROM ASL_ROLE WHERE  CONTROLLERNAME='ClosingBalance' AND COMPID='{0}' AND USERID = '{1}'",
                    ...
                    Int64 maxtranssl = Convert.ToInt64((from n in db.GlMasterDbSet
                        where
                            n.COMPID == companyID && n.TRANSDT == model.AGlMaster.TRANSDT &&
                            n.TRANSTP == model.AGlMaster.TRANSTP
                ...
                {
                    TempData["message"] = "Check Date Or Account Name";
            ...
            if (command == "Update")
            {
                if (model.AGlMaster.TRANSDT != null && (model.AGlMaster.DEBITCD != 0 && model.AGlMaster.DEBITCD != null))
                {
                    ...
                    foreach (var n in query)
                    {
                        ...
                        n.INSIPNO = model.AGlMaster.INSIPNO;
                        n.INSTIME = model.AGlMaster.INSTIME;
                        n.INSLTUDE = model.AGlMaster.INSLTUDE;
                        n.INSUSERID = model.AGlMaster.INSUSERID;
        ...
        public JsonResult ItemNameChanged(string changedText)
        {
            ...
            var rt2 = db.GlAcchartDbSet.Where(n => n.ACCOUNTNM == itemId &&
                                                         n.COMPID == compid).Select(n => new
                                                         {
                                                             debitcd = n.ACCOUNTCD,
